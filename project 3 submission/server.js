...
function loadDatabase(){
  try {
  var doc = yaml.safeLoad(fs.readFileSync('database.yaml', 'utf8'));
  return(doc);
} catch (e) {
  console.log(e);
}
...
function saveDatabase(){
  data = yaml.safeDump (database, {
    'styles': {
    '!!null': 'uppersave' // dump null as NULL
    },
  'sortKeys': false        // sort object keys
  });

  //write yaml to disk
  fs.writeFile("database.yaml", data, function(err) {
    if(err){
...
function createComment(url,request) {
  ...
  // request must have both a body before we can get the comment attribute
  const requestComment = request.body && request.body.comment;
  const response = {};

 if (requestComment && requestComment.articleId && database.articles[requestComment.articleId] && requestComment.body && requestComment.username && database.users[requestComment.username]) {
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    //remove commentId from article commentIds array
    ...
    //remove commentIds from users commentIds array
...
function upvoteComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  //request must have a body before we can get the username attribute
  const username = request.body && request.body.username
  const savedComment = database.comments[id]
  const response = {};


//username must exit in request body. username must exist in database. comment must exist in database. request must have body.
if (username && database.users[username] && database.comments[id]) {
  const upvotes = database.comments[id].upvotedBy;
  const downvotes = database.comments[id].downvotedBy;
  //check for existing upvote with this username. Can only upvote once.
...
function downvoteComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  //request must have a body or we cant get the username attribute
  const username = request.body && request.body.username
  const savedComment = database.comments[id]
  const response = {};


//username must exit in request body. username must exist in database. comment must exist in database.
if (username && database.users[username] && database.comments[id]) {
  const upvotes = database.comments[id].upvotedBy;
  const downvotes = database.comments[id].downvotedBy;
  //check for existing downvote with this username. Can only downvote once.
